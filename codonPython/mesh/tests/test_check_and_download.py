...
# TESTS:
# All tests need to be run in both save to folder and yield as generator mode
# Raises error if there were any errors
# Error raised contains all raised errors
# Including in recursive mode
# Successful messges are still returned
# calls ack on successful messages
# does not call ack on failed messages
...
def mock_download(message_id, save_folder=None):
    ...
    return {message_id: {"data": message_id, "headers": {}}}
...
def test_CheckDownload_DownloadsCorrectGenerator(patch_valid, track_ack):
    ...
    msg = next(out)
    assert msg == {"1": {"data": "1", "headers": {}}}
    msg = next(out)
    assert msg == {"2": {"data": "2", "headers": {}}}
    assert track_ack.data == [(("1",), {})]
    msg = next(out)
    assert msg == {"3": {"data": "3", "headers": {}}}
...
def test_CheckDownload_RecurseGen(patch_recurse, track_ack):
    ...
    assert track_ack.data == [(("1",), {}), (("2",), {})]
    msg = next(out)
    assert msg == {"4": {"data": "4", "headers": {}}}
    assert track_ack.data == [(("1",), {}), (("2",), {}), (("3",), {})]
    msg = next(out)
    assert msg == {"5": {"data": "5", "headers": {}}}
...
def test_CheckDownload_ErrorsRecurseGen(patch_errors, track_ack):
    ...
    assert msg == {"5": {"data": "5", "headers": {}}}
    msg = next(out)
    assert msg == {"7": {"data": "7", "headers": {}}}
    assert track_ack.data == [(("5",), {})]
    msg = next(out)
    assert msg == {"8": {"data": "8", "headers": {}}}
